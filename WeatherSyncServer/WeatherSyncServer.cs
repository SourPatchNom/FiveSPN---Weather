...
namespace WeatherSyncServer
{
    public class WeatherSyncServer : BaseScript
    {
        ...
        public WeatherSyncServer()
        {
            ...
            {
                TriggerEvent("FiveSPN-LogToServer", API.GetCurrentResourceName(),1,$"Weather location is not set in the resource manifest, unable to start the resource!");
                return;
            }
            
            TriggerEvent("FiveSPN-LogToServer", API.GetCurrentResourceName(),4,$"Server weather is now set to {_weatherLocation}.");
        ...
        private void SetOverride([FromSource]Player player, string newWeather)
        {
            ...
            lock (_weatherLock)
            {
                ...
                _forceUpdate = true;
                TriggerEvent("FiveSPN-LogToServer", API.GetCurrentResourceName(),4,$"Weather is now set to {_serverWeather} by {player.Name}.");
        ...
        private void SetLocationZip([FromSource]Player player, string newLocation)
        {
            if (!CheckPermsNow(player))
            {
                TriggerEvent("FiveSPN-LogToServer", API.GetCurrentResourceName(),4,$"Weather location requested to be {newLocation} by {player.Name} but player is not an admin.");
                return;
            }
            lock (_weatherLock)
            {
                ...
                _forceUpdate = true;
                _weatherApiCallType = WeatherApiCallType.ZipCode;
                TriggerEvent("FiveSPN-LogToServer", API.GetCurrentResourceName(),4,$"Weather location is now set to {newLocation} by {player.Name}.");
        ...
        {
            //TriggerEvent("FiveSPN-LogToServer", API.GetCurrentResourceName(),4,$"Admin permission check requested by {player.Name}."));
        ...
        private void SetRefresh([FromSource]Player player, int newRefreshRate)
        {
            if (!CheckPermsNow(player))
            {
                TriggerEvent("FiveSPN-LogToServer", API.GetCurrentResourceName(),4,$"Weather refresh rate update requested to be {newRefreshRate} by {player.Name} but player is not an admin.");
                return;
            }
            lock (_weatherLock)
            {
                _refreshRate = newRefreshRate > 1 ? newRefreshRate : 1;
                TriggerEvent("FiveSPN-LogToServer", API.GetCurrentResourceName(),4,$"Weather refresh rate is now set to {newRefreshRate} by {player.Name}.");
        ...
        private async Task OnTick()
        {
            if (((DateTime.Now - _lastUpdateTime).Minutes > _refreshRate || _forceUpdate) && !_currentlyUpdating)
            {
                ...
                if (newWeather != _serverWeather)
                {
                    ...
                    TriggerEvent("FiveSPN-LogToServer", API.GetCurrentResourceName(),4,$"Weather is now set to {_serverWeather}, which is synced from {(_forceWeather ? _forceWeatherSource : _weatherLocation)}.");
                    TriggerClientEvent("WeatherSync:UpdateClientWx", _serverWeather, _forceWeather ? _forceWeatherSource : _weatherLocation);
                }
